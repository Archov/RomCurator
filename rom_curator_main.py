...
def _default_ingestion_settings() -> Dict[str, Any]:
    """Return default ingestion settings."""
...
class RomCuratorConfig(BaseModel):
    ...
    ingestion_settings: Dict[str, Any] = Field(
        default_factory=_default_ingestion_settings
    )
    ...
    @field_validator(
        "database_path", "importer_scripts_directory", "log_directory", mode="before"
    )
    @classmethod
        ...
        if not parent.exists():
            if auto_create:
                try:
                    ...
                    raise ValueError(
                        f"Unable to create directory '{parent}': {exc}"
                    ) from exc
            else:
                raise ValueError(
        ...
        if not value.exists():
            if auto_create:
                try:
                    ...
                    raise ValueError(
                        f"Unable to create directory '{value}': {exc}"
                    ) from exc
            else:
                raise ValueError(
...
class ConfigManager:
    ...
    def _load_model(self) -> RomCuratorConfig:
        ...
        try:
            ...
            self._logger.error("Failed to read configuration: %s", exc)
        ...
        try:
            ...
            self._logger.error("Configuration validation failed: %s", error)
            self._show_error_dialog("Invalid Configuration", self._format_validation_errors(error))
            default_model = RomCuratorConfig()
            self._write_config_file(default_model)
            return default_model
    ...
    def set(self, key: str, value: Any) -> None:
        ...
        try:
            ...
            self._logger.error("Invalid configuration update for '%s': %s", key, error)
            self._show_error_dialog("Invalid Configuration", self._format_validation_errors(error))
            return
    ...
    def save(self) -> None:
        ...
        try:
            ...
            self._logger.error("Configuration invalid during save: %s", error)
            self._show_error_dialog("Invalid Configuration", self._format_validation_errors(error))
            return
    ...
    def ensure_directories(self) -> None:
        ...
        if not self.get('auto_create_directories', True):
            return
    ...
    def _write_config_file(self, config_model: RomCuratorConfig) -> None:
        ...
        try:
            ...
            self._logger.error("Could not save config file: %s", exc)

    @staticmethod
    def _show_error_dialog(title: str, message: str) -> None:
        """Display an error dialog if a QApplication is active."""
